...
export class QuoteBuilderComponent {
  ...
  activeProductId = signal<string>(PRODUCTS[0].id);
  ...
  unitBasePrice = computed(() => {
    ...
    return this.activeProduct().basePrice + embroideryPrice;
  });
  ...
  finalUnitPrice = computed(() => {
    return this.finalSubtotal() / this.quantity();
  });
  ...
  constructor() {
    effect(() => {
      ...
      if (prodInput) {
        untracked(() => {
          this.activeProductId.set(prodInput.id);
        });
      }
    });
